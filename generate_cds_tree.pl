...
(@ARGV==3) || die "usage: $0 <num_of_nodes> <sensing_nodes%> <density%>\n";
...
foreach my $n (keys %nodes){
	if (($m{$n} == 0) && (!grep {$_ eq $n} @sensing_nodes)){
		delete $m{$n};
		delete $nodes{$n};
	}
}
...
printf "# Graph: %s\n", $sptg;
printf "# %s\n", '$Id: generate_cds_tree.pl 68 2013-11-22 15:15:19Z jim $';
exit 0;
